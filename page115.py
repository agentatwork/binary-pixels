...
def main():
    ...
    claim = json.load(open("claim325-result.json"))
    ...
    assert claim["issuer"].lower() == proof["owner"].lower(), "the claim is not mine"
    assert claim["uri"].endswith("binary-pixels-115-card.json"), claim["uri"]
...
the flattering one, so both are here, with the flattering one labelled. Filed as
<a href="https://basescan.org/tx/{claim["tx"]}">claim {claim["claim_id"]}</a> on 28 August; the
<a href="/notes/binary-pixels-115-card.json">card metadata</a> the claim points at is generated by
<code>gen_card.py</code> from the same three files as this page.
</div>
